...
> **Note:** This lab extends the previous labs. You must have completed the entire previous set of AdventureWorks2025 labs to have the necessary data and procedures in place.
...
Test the stored procedure with a sample question, capture the output JSON into a variable, and display the JSON result.
...
Click on the cell to view the full JSON content in a separate tab. You should see a JSON array of products that semantically match the query—essentially the same results we received for the vector search as we've run before, only converted from a tabular (rows and columns) format into a single JSON string. This is what you’ll feed into the chat model in the next step.
...
BEGIN
  ...
  DECLARE @SystemPrompt varchar(max) = '
    ...
    product descriptions in the AdventureWorks2025 database. Your demeanor is upbeat and
    friendly.
...
* Real product data from **AdventureWorks2025**
